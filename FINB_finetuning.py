...
from model.finetune.FINB_finetuning import FINBFinetuning
...
class Tester(object):
    ...
    def __init__(self):
        ...
        self.sourcedataset=self.get_sourcedataset(self.config.sourcedataset)
        self.dataloader = self.get_dataloader(self.config.dataset)

        self.sourcedataloader = self.get_dataloader(self.config.sourcedataset)
    ...
    #添加
    def get_sourcedataset(self, config):
    ...
    def validate(self):
        val_bar = tqdm(zip(self.dataloader, self.sourcedataloader),ncols=100,total=self.config.dataset.trail)
        for data, source_data in val_bar:
            self.task_validate(data,source_data)
            val_bar.set_description(f'Testing')
            val_bar.set_postfix(acc=self.average_meters['acc'].avg)

    def task_validate(self, data,source_data):
        ...
        sou_images, sou_labels = self.to_device(source_data['img']), self.to_device(source_data['label'])
        if images.shape[0] == way*(shot+query_shot) and  sou_images.shape[0]== way*(shot+query_shot) :
            self.finetuning.meta_finetuning(images, sou_images,self.config.finetuning)
